...
`effectDetail`, `enableMoveTweens`, `enableHoverTweens`). `CardgameScene`
...
clears data and interaction listeners, disables input, zeroes hit areas, and
resets drag state, alpha, depth, scale, rotation, and active tweens before an
outer container can be reused. Interaction cleanup must preserve Phaser's
internal `destroy` listener so the parent layer can remove an evicted child.
Disabled Phaser interactive objects are retained
...
- **Use the reduced quality tier on phone-sized viewports.** The resolved
  `PhaserQualityProfile.effectDetail` decides this (full detail only on
  high-tier, non-phone viewports). It lowers particle counts and overdraw but
  must not alter queue ordering, duration, completion, or disabled-animation
  semantics.
