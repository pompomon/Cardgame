...
- `monochrome` is also a **raster** style — it ships cartoon-cat PNGs (one unique cat per land) rendered the same way as `hd`. The procedural pixel template is retained as the `onerror` / missing-texture fallback.
...
This (re)writes `public/cards/hd/*.png` and `public/cards/monochrome/*.png`. Re-running on the same checkout yields byte-identical output. If you change the recipe in `scripts/generate-card-art.mjs`, regenerate and commit the resulting PNGs alongside the change.
