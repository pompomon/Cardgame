...
describe('Three retained-card motion', () => {
  it('eases positions and dimensions while picking and anchors follow the displayed geometry', () => {
    ...
    const [top, bottom, left, right] = children[1].children
    expect(top.scale.x).toBe(anchor.width + 10)
    expect(top.position.y).toBe((anchor.height + 8) / 2)
    expect(bottom.position.y).toBe(-top.position.y)
    expect(right.position.x).toBe((anchor.width + 8) / 2)
    expect(left.position.x).toBe(-right.position.x)
    expect(top.scale.y).toBe(2)
    expect(right.scale.x).toBe(2)
    registry.advance(175)
    expect(registry.animating).toBe(false)
